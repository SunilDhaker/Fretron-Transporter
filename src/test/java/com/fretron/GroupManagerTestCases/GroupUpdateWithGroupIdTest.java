...
public class GroupUpdateWithGroupIdTest {


    /*
   Test of success updation of group if group id  match
    */
    @ClassRule
    ...
    public void startTest() throws InterruptedException {
        ...
        Groups existingGroup=new Groups("123","101",null,"fretron",getAdmin(),null);
        Groups updateGroup=new Groups(null,"101",null,"fretron1",list,null);
        ...
        new Thread(()-> {
            ...
            Future<RecordMetadata> md = commandProducer.send(new ProducerRecord<String, Command>(commandResultTopic, UUID.randomUUID().toString(), command1));
                ...
                Command command2 = new Command( "group.create.success",
                        ByteBuffer.wrap(groupSerde.serializer().serialize(groupTopic,existingGroup)),
                        UUID.randomUUID().toString(),
                        200,
                        null,
                        12345678902L,
                        System.currentTimeMillis());
                Producer<String, Command> commandProducer2= HelperClass.getProducer(CLUSTER.bootstrapServers(),CLUSTER.schemaRegistryUrl());
                Future<RecordMetadata> md2 =commandProducer2.send(new ProducerRecord<String, Command>(commandResultTopic , UUID.randomUUID().toString(), command2));
            ...
            Command command = new Command( "group.update.command",
                    ByteBuffer.wrap(groupSerde.serializer().serialize(groupTopic,updateGroup)),
                    UUID.randomUUID().toString(),
                    200,
                    null,
                    12345678902L,
        ...
        List<Command> actual =  IntegrationTestUtils.waitUntilMinValuesRecordsReceived(HelperClass.getConsumerProps("my-group-test" , CLUSTER) ,commandResultTopic,4, 120000);
