...
public class GroupManager {
    ...
    SpecificAvroSerde<CommandOfModel> CommandOfModelSerde ;
    ...
    public  void createGroup(KStream<String,Command> commandKStream,KTable<String,Transporter> transporterKTable)
    {
        ...
        KStream<String,CommandOfModel> commandOfGroupsAndTransporterKStream = createGroupKStream
                .mapValues((values) -> {
                    CommandOfModel groupAndCommand = new CommandOfModel();
        ...
        KStream<String, CommandOfModel> joinedKStream = commandOfGroupsAndTransporterKStream
                .selectKey((key, value) -> value.group.getTransporterId())
                .leftJoin(transporterKTable,
                        (leftValue, rightValue) -> new CommandOfModel(leftValue.command,null, rightValue, leftValue.group),
                        stringSerde, CommandOfModelSerde);

        /*
        branch joined k stream
         */
        KStream<String, CommandOfModel>[] branchedStream = joinedKStream.branch((k, v) -> v.transporter == null, (k, v) -> v.transporter != null);
        ...
        */
        ...
        KStream<String,CommandOfModel> joinedGroupSuccessKStream =groupCreateSuccess
                .mapValues(value->{
                    CommandOfModel groupsAndTransporter=new CommandOfModel();
    ...
    public  void updateGroup(KStream<String,Command> commandKStream,KTable<String,Groups> groupsKTable,KTable<String,Transporter>transporterKTable)
    {
        ...
        KStream<String, CommandOfModel> commandOfGroupsKStream = commandKStream
                .filter((key, value) -> value.getType().contains("update"))
                .mapValues((value) ->
                {
                    CommandOfModel commandOfGroups=new CommandOfModel();
        ...
        KStream<String,CommandOfModel> joinedKStream=commandOfGroupsKStream
                .leftJoin(groupsKTable,(leftValue,rightValue)->new CommandOfModel(leftValue.command,null,null,rightValue),
                        stringSerde,CommandOfModelSerde);
        KStream<String,CommandOfModel>[] branchedjoinedKStream=joinedKStream.branch((key,value)->value.group==null ,(key,value)->value.group!=null);
        ...
        KStream<String,CommandOfModel> joinedGroupSuccessKStream =groupUpdateSuccess
                .mapValues(value->{
                    CommandOfModel groupsAndTransporter=new CommandOfModel();
                    groupsAndTransporter.setCommand(value);
                    groupsAndTransporter.setGroup( groupSpecificAvroSerde.deserializer().deserialize(Context.getConfig().getString(Constants.KEY_GROUP_TOPIC), value.getData().array()));
